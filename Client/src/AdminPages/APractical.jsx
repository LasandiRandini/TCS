...
//       setPracticalId(response.data.practical_id);
...
//             </button>
//             {practical_id && (
//                                 <Link to={`/AddTimeslots/${practical_id}`} className="bg-yellow-500 text-white font-bold py-1 px-2 rounded ml-2">Go to Quiz</Link>
//                             )}
//             {error && <p className="text-red-500 text-xs italic mt-4 text-center">{error}</p>}
//           </form>
//         </div>
...
const APractical = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:8800/api/practicals/practical', inputs);
      setPracticalId(response.data.practical_id);
      navigate(`/AddTimeslots/${response.data.practical_id}`);
    } catch (err) {
      setError(err.response.data.error);
    }
  };
