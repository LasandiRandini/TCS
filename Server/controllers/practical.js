...
export const practical = async (req, res) => {
  ...
  try {
    ...
    if (result.affectedRows === 1) {
      ...
      res
        .status(201)
        .json({ message: "Practical created successfully", practicalId });
    } else {
...
export const getpractical = async (req, res) => {
  try {
    const query =
      "SELECT practical_id,year, title, date, duration, institute, description FROM practical";
    db.query(query, (err, results) => {
      if (err) {
        console.error("Error fetching practical data:", err);
        res
          .status(500)
          .json({ error: "An error occurred while fetching practical data" });
      } else {
        console.log(results);
...
export const deletePractical = async (req, res) => {
  try {
    ...
    db.query(sql, [practical_id], (err, data) => {
      ...
      return res
        .status(200)
        .json({ message: "Practical deleted successfully" });
    });
  } catch (error) {
...
export const addTimeSlots = async (req, res) => {
  ...
  if (!practical_id || !time_slots || !max_limit) {
    return res
      .status(400)
      .json({ error: "Practical ID, time slots, and max limit are required" });
  }

  const timeSlotsArray = time_slots.map((slot) => [
    slot,
    practical_id,
    0,
    max_limit,
  ...
  try {
    const query =
      "INSERT INTO practicaltimeslots (time_slots, practical_id, votes_count, max_limit) VALUES ?";
    ...
    db.query(query, [timeSlotsArray], (err, result) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ error: "An error occurred while inserting time slots" });
      }
  ...
  try {
    ...
    db.query(sql, [practical_id], (err, results) => {
      ...
      if (results.length === 0) {
        return res
          .status(404)
          .json({ error: "No time slots found for the given practical ID" });
      }
...
export const voteForTimeSlot = (req, res) => {
  ...
  if (!student_id || !slot_id) {
    return res
      .status(400)
      .json({ error: "Student ID and Slot ID are required" });
  }

  const timeSlotSql =
    "SELECT votes_count, max_limit FROM practicaltimeslots WHERE slot_id = ?";
  ...
  db.query(timeSlotSql, [slot_id], (err, timeSlotResult) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ error: "An error occurred while fetching time slot data" });
    }
    ...
    if (votes_count >= max_limit) {
      return res
        .status(400)
        .json({ error: "මෙම practical session එක සදහා time slots full වී ඇත" });
    }

    const voteSql =
      "INSERT INTO practicalvotes (student_id, slot_id) VALUES (?, ?)";
    ...
    db.query(voteSql, [student_id, slot_id], (err) => {
      ...
      const updateVoteCountSql =
        "UPDATE practicaltimeslots SET votes_count = votes_count + 1 WHERE slot_id = ?";
      ...
      db.query(updateVoteCountSql, [slot_id], (err) => {
        if (err) {
          ...
          return res
            .status(500)
            .json({ error: "An error occurred while updating vote count" });
        }
...
export const updatePractical = async (req, res) => {
  ...
  try {
    ...
    db.query(sql, params, (err, result) => {
      ...
      return res
        .status(200)
        .json({ message: "Practical updated successfully" });
    });
  } catch (error) {
